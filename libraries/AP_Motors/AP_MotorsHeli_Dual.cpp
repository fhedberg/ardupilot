...
bool AP_MotorsHeli_Dual::allow_arming() const
{
    // returns false if main rotor speed is not zero
    if (_rsc_mode != AP_MOTORS_HELI_RSC_MODE_NONE && _rotor.get_estimated_speed() > 0) {
        return false;
    }
...
void AP_MotorsHeli_Dual::move_swash(int16_t roll_out, int16_t pitch_out, int16_t collective_in, int16_t yaw_out)
{
    ...
    if (_servo_manual == 1) {      // are we in manual servo mode? (i.e. swash set-up mode)?
        ...
        collective_out_scaled = collective_in * _collective_scalar + _throttle_radio_min - 1000;
    ...
    // swashplate servos
    _servo_1.servo_out = (_rollFactor[CH_1] * roll_out + _pitchFactor[CH_1] * pitch_out + _yawFactor[CH_1] * yaw_out)/10 + _collectiveFactor[CH_1] * collective_out_scaled + (_servo_1.radio_trim-1500);
    _servo_2.servo_out = (_rollFactor[CH_2] * roll_out + _pitchFactor[CH_2] * pitch_out + _yawFactor[CH_2] * yaw_out)/10 + _collectiveFactor[CH_2] * collective_out_scaled + (_servo_2.radio_trim-1500);
    _servo_3.servo_out = (_rollFactor[CH_3] * roll_out + _pitchFactor[CH_3] * pitch_out + _yawFactor[CH_3] * yaw_out)/10 + _collectiveFactor[CH_3] * collective_out_scaled + (_servo_3.radio_trim-1500);
    _servo_4.servo_out = (_rollFactor[CH_4] * roll_out + _pitchFactor[CH_4] * pitch_out + _yawFactor[CH_4] * yaw_out)/10 + _collectiveFactor[CH_4] * collective_out_scaled + (_servo_4.radio_trim-1500);
    _servo_5.servo_out = (_rollFactor[CH_5] * roll_out + _pitchFactor[CH_5] * pitch_out + _yawFactor[CH_5] * yaw_out)/10 + _collectiveFactor[CH_5] * collective_out_scaled + (_servo_5.radio_trim-1500);
    _servo_6.servo_out = (_rollFactor[CH_6] * roll_out + _pitchFactor[CH_6] * pitch_out + _yawFactor[CH_6] * yaw_out)/10 + _collectiveFactor[CH_6] * collective_out_scaled + (_servo_6.radio_trim-1500);
