...
const AP_Param::GroupInfo AP_MotorsHeli_Single::var_info[] PROGMEM = {
    ...
    // @User: Standard
    AP_GROUPINFO("FLYBAR_MODE", 8, AP_MotorsHeli_Single, _flybar_mode, AP_MOTORS_HELI_NOFLYBAR),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("TAIL_SPEED", 9, AP_MotorsHeli_Single, _tail_direct_drive_speed, AP_MOTORS_HELI_SINGLE_TAIL_DIRECTDRIVE_SPEED_DEFAULT),
...
{
    return _main_rotor.get_desired_speed() == 0;
}
...
void AP_MotorsHeli_Single::set_desired_rotor_speed(int16_t desired_speed)
{
    ...
    if (desired_speed > 0 && _tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_DIRECTDRIVE_VARPITCH) {
        _tail_rotor.set_desired_speed(_tail_direct_drive_speed);
    } else {
...
int16_t AP_MotorsHeli_Single::get_desired_rotor_speed() const
{
    return _main_rotor.get_desired_speed();
}

// get_estimated_rotor_speed
int16_t AP_MotorsHeli_Single::get_estimated_rotor_speed()
{
    return _main_rotor.get_estimated_speed();
}
...
void AP_MotorsHeli_Single::move_swash(int16_t roll_out, int16_t pitch_out, int16_t collective_in, int16_t yaw_out)
{
    ...
    if (_servo_manual == 1) {      // are we in manual servo mode? (i.e. swash set-up mode)?
        ...
        collective_out_scaled = collective_in * _collective_scaler + _throttle_radio_min - 1000;
